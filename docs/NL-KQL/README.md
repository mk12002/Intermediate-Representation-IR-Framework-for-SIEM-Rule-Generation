...
| Document | Covers |
|---|---|
| [`docs/MASTER_PLAN.md`](docs/MASTER_PLAN.md) | **Start here for full depth.** Complete planning + implementation reference in one file — motivation through future work |
| [`docs/architecture.md`](docs/architecture.md) | Full pipeline specs, agent prompts, IR schema, repair loop logic, a worked end-to-end example |
| [`docs/dataset.md`](docs/dataset.md) | Full construction methodology, paraphrasing guidelines, verification rubric, complexity tagging |
| [`docs/evaluation.md`](docs/evaluation.md) | Exact metric definitions, ablation protocol, statistical methodology, logic-correctness rubric |
| [`docs/kql-asim-primer.md`](docs/kql-asim-primer.md) | KQL/ASIM technical reference scoped to what this project uses |
...
> 📄 For the full KQL/ASIM technical background behind this taxonomy — operators, naming conventions, and why KQL is easily confused with SQL/SPL — see [`docs/kql-asim-primer.md`](docs/kql-asim-primer.md).
...
> 📄 Full specification — agent prompts, schema validator logic, the repair loop's code, and a worked end-to-end example — in [`docs/architecture.md`](docs/architecture.md).
...
> 📄 Full methodology — step-by-step procedure, paraphrasing guidelines, the manual verification rubric, and complexity-tagging criteria — in [`docs/dataset.md`](docs/dataset.md).
...
> 📄 Exact metric formulas, the full ablation protocol, statistical methodology (bootstrap CIs, McNemar's test), and the logic-correctness scoring rubric — in [`docs/evaluation.md`](docs/evaluation.md).
